        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    <x-nav-link 
                        href="{{ auth()->check() ? (auth()->user()->isOwner() ? route('admin.dashboard') : (auth()->user()->isTrainer() ? route('trainer-panel.dashboard') : route('trainers'))) : route('home') }}"
                        wire:navigate>
                        {{ __('Главная') }}
                    </x-nav-link>
                </div>
            ...
            <div class="hidden sm:flex sm:items-center sm:ms-6">
                <x-dropdown align="right" width="48">
                    ...
                    <x-slot name="content">
                        <x-dropdown-link 
                            href="{{ auth()->check() ? (auth()->user()->isOwner() ? route('admin.profile') : (auth()->user()->isTrainer() ? route('trainer-panel.profile') : route('profile'))) : route('login') }}" 
                            wire:navigate>
                            {{ __('Профиль') }}
                        </x-dropdown-link>
